...
AUGMENT_LC = True  # Apply small augmentations (time shift + jitter) to LC training data
...
def build_hybrid_model(tab_features_count, n_classes, reg_outputs_count, lc_len):
    ...
    # small L2 regularization on Dense layers helps generalization
    from tensorflow.keras import regularizers
    tab_branch = Dense(64, activation="relu", kernel_regularizer=regularizers.l2(1e-4))(tab_inp)
    ...
    lc_branch = Conv1D(32, 5, activation="relu", padding="same", kernel_regularizer=regularizers.l2(1e-4))(lc_inp)
    lc_branch = BatchNormalization()(lc_branch)
    lc_branch = MaxPooling1D(4)(lc_branch)
    lc_branch = Dropout(0.3)(lc_branch)
    lc_branch = Conv1D(64, 5, activation="relu", padding="same", kernel_regularizer=regularizers.l2(1e-4))(lc_branch)
    lc_branch = BatchNormalization()(lc_branch)
    lc_branch = MaxPooling1D(4)(lc_branch)
    lc_branch = Dropout(0.3)(lc_branch)
...
def train_combined_model_fixed():
    ...
    # Apply log1p to heavily skewed continuous regression targets (koi_prad, Distance_AU)
    # We'll transform columns 0 (koi_prad) and 1 (Distance_AU) with log1p before scaling
    y_reg_cont = y_reg.copy()
    if y_reg_cont.shape[0] > 0:
        # avoid negative values, ensure non-negative for log1p
    ...
    # Re-apply the log1p transform to the train/test splits of regression targets
    def apply_log1p_to_split(y_split):
        y_copy = y_split.copy()
        if y_copy.shape[0] > 0:
            y_copy[:, 0] = np.where(y_copy[:, 0] >= 0, np.log1p(y_copy[:, 0]), y_copy[:, 0])
            y_copy[:, 1] = np.where(y_copy[:, 1] >= 0, np.log1p(y_copy[:, 1]), y_copy[:, 1])
        return y_copy

    y_reg_train = apply_log1p_to_split(y_reg_train)
    y_reg_test = apply_log1p_to_split(y_reg_test)

    # Fit scaler on regression targets using training split
    ...
    # Normalize sample weights so their mean is 1 (prevents scaling the loss unintentionally)
    sample_weights_train_cls = np.array([float(w) for w in sample_weights_train_cls])
    sample_weights_train_reg = np.array([float(w) for w in sample_weights_train_reg])
    if sample_weights_train_cls.mean() > 0:
        sample_weights_train_cls = sample_weights_train_cls / sample_weights_train_cls.mean()
    if sample_weights_train_reg.mean() > 0:
        sample_weights_train_reg = sample_weights_train_reg / sample_weights_train_reg.mean()

    sample_weight_for_fit = [sample_weights_train_cls, sample_weights_train_reg]

    # Build model
    model = build_hybrid_model(len(TABULAR_FEATURES), n_classes, len(regression_targets), LC_LEN)
    # Use a fixed optimizer with an explicit learning rate; regularization added in layers above
    # Use a smaller LR and reduce reg loss contribution to avoid domination of MSE
    optimizer = tf.keras.optimizers.Adam(learning_rate=1e-4)
    model.compile(
        optimizer=optimizer,
        loss={"class_output": "sparse_categorical_crossentropy", "reg_output": "mse"},
        loss_weights={"class_output": 0.85, "reg_output": 0.15},
    ...
    # Simple augmentation for LC training samples (time shift + small jitter)
    def augment_lcs(X_lc, n_augs=1, max_shift=8, jitter_scale=0.002):
        aug_list = []
        for i in range(len(X_lc)):
            ...
            for _ in range(n_augs):
                ...
                x_shift = x_shift + np.random.normal(0, jitter_scale, size=x_shift.shape)
                aug_list.append(x_shift)
        if len(aug_list) == 0:
            return np.zeros((0, X_lc.shape[1], 1))
        arr = np.array(aug_list)
        return np.expand_dims(arr, axis=2)

    # Optionally augment training set to improve robustness
    if AUGMENT_LC:
        np.random.seed(RANDOM_SEED)
        n_augs = 1  # number of augmented copies per sample (small to start)
        X_lc_aug = augment_lcs(X_lc_train, n_augs=n_augs)
        if X_lc_aug.shape[0] > 0:
            X_tab_aug = np.repeat(X_tab_train, n_augs, axis=0)
            y_class_aug = np.repeat(y_class_train, n_augs)
            y_reg_aug = np.repeat(y_reg_train, n_augs, axis=0)
            sample_weights_aug_cls = np.repeat(sample_weights_train_cls, n_augs)
            sample_weights_aug_reg = np.repeat(sample_weights_train_reg, n_augs)

            # Concatenate augmented data back into training sets
            X_tab_train = np.concatenate([X_tab_train, X_tab_aug], axis=0)
            X_lc_train = np.concatenate([X_lc_train, X_lc_aug], axis=0)
            y_class_train = np.concatenate([y_class_train, y_class_aug], axis=0)
            y_reg_train = np.concatenate([y_reg_train, y_reg_aug], axis=0)
            sample_weights_train_cls = np.concatenate([sample_weights_train_cls, sample_weights_aug_cls], axis=0)
            sample_weights_train_reg = np.concatenate([sample_weights_train_reg, sample_weights_aug_reg], axis=0)

    # Rebuild the sample weight structure matching outputs
    # Ensure regression targets are re-scaled after augmentation so lengths match
    try:
        y_reg_train_scaled = reg_scaler.transform(y_reg_train)
    except Exception:
        # fallback: if something goes wrong, leave as-is (shouldn't happen)
        y_reg_train_scaled = reg_scaler.transform(y_reg_train)

    # Re-normalize sample weights so their mean is 1 after augmentation
    sample_weights_train_cls = np.array([float(w) for w in sample_weights_train_cls])
    sample_weights_train_reg = np.array([float(w) for w in sample_weights_train_reg])
    if sample_weights_train_cls.mean() > 0:
        sample_weights_train_cls = sample_weights_train_cls / sample_weights_train_cls.mean()
    ...
    # Callbacks: early stopping, LR reduction, and best-model checkpointing
    callbacks = []
    callbacks.append(EarlyStopping(monitor='val_class_output_accuracy', patience=8, mode='max', restore_best_weights=True))
    callbacks.append(ReduceLROnPlateau(monitor='val_class_output_loss', factor=0.5, patience=4, min_lr=1e-6))
    ckpt_path = os.path.join(ARTIFACTS_DIR, 'best_combined_model.weights.h5')
    callbacks.append(ModelCheckpoint(ckpt_path, monitor='val_class_output_accuracy', save_best_only=True, save_weights_only=True, mode='max'))
    ...
    return tab_scaler, label_enc, regression_targets, model, reg_scaler
...
def run_tess_inference(tab_scaler, label_enc, regression_targets, model, reg_scaler=None):
    ...
    # If regression scaler was used during training, inverse-transform the reg outputs
    if reg_scaler is not None:
        try:
            tess_pred_reg_inv = reg_scaler.inverse_transform(tess_pred_reg)
            # continuous targets (first two) were log1p-transformed before scaling; undo that
            tess_pred_reg_inv[:, 0] = np.expm1(tess_pred_reg_inv[:, 0])
            tess_pred_reg_inv[:, 1] = np.expm1(tess_pred_reg_inv[:, 1])
            tess_pred_reg = tess_pred_reg_inv
        except Exception:
            # If inverse fails, keep raw predictions
            pass
...
# =====================
def main():
    parser = argparse.ArgumentParser(description='Run TF combined pipeline and/or PyTorch hybrid script')
    parser.add_argument('--mode', choices=['tf', 'pytorch', 'both'], default='tf', help='Which pipeline to run')
    # options for the PyTorch script when run as subprocess
    parser.add_argument('--pytorch-samples', type=int, default=2000, help='Number of samples for PyTorch quick run')
    parser.add_argument('--pytorch-checkpoint', type=str, default='hybrid_from_trial.pt', help='Checkpoint path for PyTorch script')
    parser.add_argument('--pytorch-use-sampler', action='store_true', help='Pass --use-sampler to PyTorch training')
    parser.add_argument('--pytorch-mode', choices=['train','serve'], default='train', help='mode to pass to PyTorch script')

    args = parser.parse_args()

    # run PyTorch script in a separate process to avoid importing torch in this TF process
    def run_pytorch_subprocess():
        script_path = os.path.join(os.path.dirname(__file__), 'hybrid_exovision_improved.py')
        cmd = [sys.executable, script_path, '--mode', args.pytorch_mode, '--checkpoint', args.pytorch_checkpoint, '--samples', str(args.pytorch_samples)]
        if args.pytorch_use_sampler:
            cmd.append('--use-sampler')
        print('\nRunning PyTorch script: ' + ' '.join(cmd) + '\n')
        try:
            subprocess.run(cmd, check=True)
        except subprocess.CalledProcessError as e:
            print(f"PyTorch script failed with exit code {e.returncode}")

    if args.mode == 'pytorch':
        run_pytorch_subprocess()
    elif args.mode == 'tf':
        ...
        # run PyTorch first (as a background/prep step), then TF pipeline
        run_pytorch_subprocess()
        tab_scaler, label_enc, regression_targets, model, reg_scaler = train_combined_model_fixed()
        run_tess_inference(tab_scaler, label_enc, regression_targets, model, reg_scaler=reg_scaler)
